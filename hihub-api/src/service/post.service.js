...
class PostService {
    ...
    // 分页查询帖子
    async getPosts(searchWord, startNum, pageSize, tags) {debugger
        const sqlBasic = `select p.*, GROUP_CONCAT(rpt.tagid) as tagIds, w.url as weburl from post p`
        + ` left join rel_post_tag rpt on rpt.postid = p.id`
        + ` left join website w on p.webid = w.id`
        + ` where p.name like '%${searchWord}%'` 
        const sqlTag = sqlBasic + ` and rpt.tagid in (${tags}) GROUP BY p.id`
